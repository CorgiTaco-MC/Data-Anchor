            ...
            for (Field declaredField : obj.getClass().getDeclaredFields()) {
                ...
                if (declaringClass.isPrimitive()) {
                    if (declaringClass == boolean.class || declaringClass == Boolean.class) {
                        declaredField.set(obj, compoundTag.getBoolean(name).orElseThrow().booleanValue());
                    }
                    if (declaringClass == int.class || declaringClass == Integer.class) {
                        declaredField.set(obj, compoundTag.getInt(name).orElseThrow().intValue());
                    }
                    if (declaringClass == long.class || declaringClass == Long.class) {
                        declaredField.set(obj, compoundTag.getLong(name).orElseThrow().longValue());
                    }
                    if (declaringClass == float.class || declaringClass == Float.class) {
                        declaredField.set(obj, compoundTag.getFloat(name).orElseThrow().floatValue());
                    }
                    if (declaringClass == double.class || declaringClass == Double.class) {
                        declaredField.set(obj, compoundTag.getDouble(name).orElseThrow().doubleValue());
                    }
                    if (declaringClass == byte.class || declaringClass == Byte.class) {
                        declaredField.set(obj, compoundTag.getByte(name).orElseThrow().byteValue());
                    }
                    if (declaringClass == short.class || declaringClass == Short.class) {
                        declaredField.set(obj, compoundTag.getShort(name).orElseThrow().shortValue());
                    }
                ...
                if (declaringClass.isInstance(Collection.class)) {
                    ...
                    if (o instanceof Collection<?> collection) {
                        collection.clear();
                        ListTag list = compoundTag.getList(name).orElseThrow();
                        for (Tag tag1 : list) {
                            collection.add(fromTag(tag1, null));
                        }
                    }
                    declaredField.set(obj, compoundTag.getList(name));
                }

                if (declaredField.getType().isEnum()) {
                    declaredField.set(obj, Enum.valueOf((Class<Enum>) declaredField.getType(), compoundTag.getString(name).orElseThrow()));
                }
